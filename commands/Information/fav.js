...
return r
    ...
    if(filter.length === 0) {
      return message.channel.send("You don't have any favourite pokemon(s). Use .addfav to add a favourite pokemon.");
    }
...
//    if(args[0] == 0) args[0]; 
 ...
 //   const no = ((ix + 1)*20)-20
    ...
    if(args[0] > chunks.length){
      ...
      embed.setDescription(chunks[index].map((item, i) => `**${item.name} ${(item.shiny ? "⭐": "")}** | Level: ${item.level} | Number: ${item.num+1} | IV: ${item.totalIV}%${(item.nick != null ? ` | Nickname: ${item.nick}`: "")}`).join('\n'))
